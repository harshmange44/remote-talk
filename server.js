...
io.on('connect', socket => {
    ...
    socket.on("join room", (roomid, username) => {
        ...
        if (rooms[roomid] && rooms[roomid].length > 0) {
            ...
            socket.to(roomid).emit('message', `${username} joined the room.`, 'Remote Talk', moment(new Date().getTime()).format(
                "h:mm a"
            ));
    ...
    socket.on('message', (msg, username, roomid) => {
        io.to(roomid).emit('message', msg, username, moment(new Date().getTime()).format(
            "h:mm a"
        ));
    })
    ...
    socket.on('disconnect', () => {
        ...
        socket.to(socketroom[socket.id]).emit('message', `${socketname[socket.id]} left the room.`, `Remote Talk`, moment(new Date().getTime()).format(
            "h:mm a"
        ));
